/**
 * DELETE /api/games/:id/aliases/:playerId
 *
 * Removes the alias for the given source player. The settlement plan
 * is re-derived (the player reappears in the active roster, with their
 * original net restored).
 */

import { loadGameRow, removeAlias } from '../../../../lib/db';
import {
  OPTIONS_NO_CONTENT,
  errorResponse,
  jsonResponse,
...
export const onRequestDelete: PagesFunction<Env> = async (ctx) => {
  ...
  const updated = await removeAlias(ctx.env.DB, {
    gameId,
    playerId,
